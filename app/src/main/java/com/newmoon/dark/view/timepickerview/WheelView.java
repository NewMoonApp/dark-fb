package com.newmoon.dark.view.timepickerview;
...
public class WheelView extends View {
    ...
    private static final float RADIAN_OFFSET = 25F;// 竖排显示的弧度偏移值
    private static final float SCALE_MIN_TEXT_SIZE = 0.9f;// 弧度变化过程中缩放到的最小文本大小
    private static final float CENTERCONTENT_OFFSET = 19;//中间文字文字居中需要此偏移值
    private float centerCountEntOffset = CENTERCONTENT_OFFSET;//中间文字文字居中需要此偏移值
    ...
    public WheelView(Context context, AttributeSet attrs) {
        super(context, attrs);
        centerCountEntOffset = CENTERCONTENT_OFFSET * Dimensions.getPhoneHeight(context) / 1920f;
    ...
    private void initPaints() {
        ...
        paintCenterText.setTextSize(textSize * 1.3f);
    ...
    private void remeasure() {
        ...
        centerY = (measuredHeight + maxTextHeight) / 2.0F - centerCountEntOffset;
    ...
    public final void setAdapter(WheelAdapter adapter, boolean isMonth) {
        this.adapter = adapter;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if(label != null) {
            ...
            //靠右并留出空隙
            canvas.drawText(label, drawRightContentStart - centerCountEntOffset, centerY, paintCenterText);
        }
        counter = 0;
        while (counter < itemsVisible) {
            canvas.save();
            ...
            float alphaF = formatNumberToOneDigit((90f - Math.abs(angle)) / 90f);
            // 五十度以上的不绘制
            if (angle >= 50F || angle <= -50F) {
                canvas.restore();
            } else {
                String contentText = getContentText(visibles[counter]);
                boolean isContentNumber = isStringNumber(contentText);
                //计算开始绘制的位置
                ...
                if (isContentNumber) {
                    drawCenterContentStart += RADIAN_OFFSET * 1;
                    drawOutContentStart += RADIAN_OFFSET * 1;
                } else {
                    drawCenterContentStart += RADIAN_OFFSET * (1);
                    drawOutContentStart += RADIAN_OFFSET * (1);
                }
                ...
                if (translateY <= firstLineY && maxTextHeight + translateY >= firstLineY) {
                    ...
                    canvas.drawText(contentText, drawCenterContentStart, maxTextHeight - centerCountEntOffset, paintCenterText);
                    canvas.restore();
                    ...
                    canvas.scale(1.0F, (float) Math.sin(radian) * SCALECONTENT);
                    paintOuterText.setAlpha((int) (255 * 0.7f));
                    canvas.drawText(contentText, drawOutContentStart, maxTextHeight, paintOuterText);
                    canvas.restore();
                    ...
                    canvas.clipRect(0, 0, measuredWidth, (int) (itemHeight));
                    //canvas.scale(1.0F, (float) Math.sin(radian) * SCALECONTENT);
                    canvas.scale(1.0F, alphaF);
                    paintOuterText.setAlpha((int) (255 * 0.4f));
                    paintOuterText.setAlpha(alpha);
                    paintOuterText.setTextSize(textSize * (SCALE_MIN_TEXT_SIZE + (1 - SCALE_MIN_TEXT_SIZE) * alphaF));
                    canvas.drawText(contentText, drawOutContentStart, maxTextHeight, paintOuterText);
                    canvas.restore();
                }
                canvas.restore();
    ...
    private String getContentText(Object item) {
        ...
        contentText = convertMonthText(contentText);

        return contentText;
    }

    private String convertMonthText (String text) {
        if (!isMonth) {
            return text;
        }
        ...
        if (isStringNumber(text)) {
            try {
                intText = Integer.parseInt(text.trim());
    ...
    private float formatNumberToOneDigit(double number) {
